...
const { encrypt, generateKeyPair } = require('./crypto');
...
const PERSONA_URL = process.env.PERSONALITY_SERVICE_URL || 'http://localhost:8003';
const PAYMENT_URL = process.env.PAYMENT_SERVICE_URL || 'http://localhost:8004';

// Sessions locales (→ Redis en prod)
const sessions = {};

function getSession(userId) {
    if (!sessions[userId]) {
        const { publicKey, privateKey } = generateKeyPair();
        sessions[userId] = { companionId: 'luna', publicKey, privateKey };
    }
...
async function checkPremium(userId) {
    try {
        const resp = await axios.get(`${PAYMENT_URL}/users/${userId}/premium`);
...
bot.command('switch', async (ctx) => {
    ...
    if (!isPrem) {
        ctx.reply('🔒 Changer de companion est une fonctionnalité Premium.\n\nTape /premium pour débloquer.');
        return;
    }
    ctx.reply(
        'Change de companion :',
...
bot.command('memory', async (ctx) => {
    try {
        const resp = await axios.get(`${MEMORY_URL}/memories/${ctx.from.id}`);
...
bot.command('status', async (ctx) => {
    ...
    try {
        const resp = await axios.get(`${PERSONA_URL}/state/${userId}/${session.companionId}`);
        const s = resp.data;
...
bot.command('premium', async (ctx) => {
    ...
    try {
        const resp = await axios.post(`${PAYMENT_URL}/checkout`, { user_id: userId, username });
        ctx.reply(
...
bot.on('text', async (ctx) => {
    ...
    try {
        ...
        console.log(`[${userId}] Msg chiffré : ${encryptedInput.substring(0, 40)}...`);
